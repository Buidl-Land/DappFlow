  ...
  return (
    <div className="container px-4 py-8 mx-auto">
      ...
      <div className="grid grid-cols-1 gap-4 mb-8 md:grid-cols-3">
        <div className="p-6 rounded-xl shadow-lg bg-primary text-primary-content">
          <h2 className="text-xl font-bold">Projects Backed</h2>
          <p className="text-3xl font-bold">{participatedProjects.length}</p>
        </div>

        <div className="p-6 rounded-xl shadow-lg bg-secondary text-secondary-content">
          <h2 className="text-xl font-bold">Tasks Participated</h2>
          <p className="text-3xl font-bold">{userTasksWithDetails.length}</p>
        </div>

        <div className="p-6 rounded-xl shadow-lg bg-accent text-accent-content">
      ...
      <div className="mb-6 tabs">
        ...
        <button
          className={`tab tab-lifted ${activeTab === "tasks" ? "tab-active" : ""}`}
      ...
      {activeTab === "projects" && (
        <div className="space-y-6">
          ...
          {participatedProjects.length > 0 ? (
                ...
                <tbody>
                  {participatedProjects.map(investment => (
                    <tr key={investment.projectId}>
                      ...
                      <td>
                        <Link href={`/crowdfunding/${investment.projectId}`} className="btn btn-xs btn-outline">
                          Details
                        </Link>
                      </td>
                    </tr>
            ...
            <div className="p-8 text-center bg-base-200 rounded-box">
              <h3 className="text-xl font-bold">No investments yet</h3>
              <p className="mb-4">You haven&apos;t invested in any projects.</p>
          ...
          {participatedProjects.length > 0 && (
            <div className="p-6 mt-8 rounded-lg bg-base-200">
              <h3 className="mb-4 text-xl font-bold">Token Release Schedule</h3>
              <div className="overflow-x-auto">
                <table className="table w-full">
                  <thead>
                    <tr>
                    ...
                    {participatedProjects.flatMap(investment =>
                      investment.unlockSchedule.map((schedule, index) => (
                        <tr key={`${investment.projectId}-${index}`}>
                          ...
                          <td>{Math.round(investment.tokenAmount * (schedule.percentage / 100)).toLocaleString()}</td>
                          <td>
                            {schedule.unlocked ? (
        ...
        <div className="space-y-6">
          ...
          {userTasksWithDetails.length > 0 ? (
                ...
                <tbody>
                  {userTasksWithDetails.map(userTaskWithDetails => (
                    <tr key={userTaskWithDetails.taskId}>
                      ...
                      <td>
                        <Link href={`/tasks/${userTaskWithDetails.taskId}`} className="btn btn-xs btn-outline">
                          Details
                        </Link>
                      </td>
                    </tr>
            ...
            <div className="p-8 text-center bg-base-200 rounded-box">
              ...
              <p className="mb-4">You haven&apos;t applied for any tasks.</p>
